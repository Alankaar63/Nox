...
function App() {
  ...
  async function refreshAll() {
    ...
    setError("");
    try {
      const [dash, ws, ms, ap, rc, arch] = await Promise.all([
        api("/api/dashboard"),
        api("/api/workouts?days=30"),
        api("/api/meals"),
        api("/api/adaptive-plan"),
        api("/api/recipes"),
        api("/api/architecture/status"),
      ]);
      setDashboard(dash);
      setGoalForm((f) => ({
        ...f,
      ...
      <main className="app">
        ...
        <section className="marquee">
          <div className="marquee-track">
            {"CASSANDRA // REDISJSON // PGVECTOR // WEAVIATE // INFLUXDB // RUST AXUM // GO GRPC // FASTAPI LANGGRAPH // VLLM // KUBERNETES // ".repeat(6)}
          </div>
        </section>

        <section className="platform-band" id="platform-section">
          <div>
            <span className="kicker">full-stack nox platform</span>
            <h2>NOX is now mapped to the serious architecture.</h2>
            <p>
              Great Lock In, workout intelligence, training splits, knowledge RAG and nutrition operate as one system:
              event ingestion, hot state, distributed storage, vector memory, time-series trends and self-hosted LLM orchestration.
            </p>
          </div>
          <div className="platform-grid">
            {(architecture?.domains || ["Great Lock In", "Workout Tracker", "Training Splits & Knowledge Vault", "Nutrition Engine"]).map((domain) => (
              <article className="platform-tile" key={domain}>
                <strong>{domain}</strong>
                <span>{domain === "Great Lock In" ? "Schedule, commit, focus-mode recommendations." :
                  domain === "Workout Tracker" ? "Set logging, progressive overload, session summaries." :
                  domain.includes("Knowledge") ? "Coach splits, RAG memory, training philosophy." :
                  "Macros, diet logs, foods, diet charts."}</span>
              </article>
            ))}
          </div>
          <div className="infra-grid">
            {Object.entries(architecture?.data_layer || {}).map(([name, detail]) => (
              <div className="infra-chip" key={name}><b>{name}</b><span>{detail}</span></div>
            ))}
            {Object.entries(architecture?.compute_layer || {}).map(([name, detail]) => (
              <div className="infra-chip" key={name}><b>{name}</b><span>{detail}</span></div>
            ))}
            {Object.entries(architecture?.ai_layer || {}).map(([name, detail]) => (
              <div className="infra-chip" key={name}><b>{name}</b><span>{detail}</span></div>
            ))}
          </div>
        </section>
        ...
        <section className="quick-actions card">
          ...
          <div className="quick-actions-grid">
            ...
            <button type="button" className="ghost-btn" onClick={() => jumpTo("platform-section")}>View Architecture</button>
